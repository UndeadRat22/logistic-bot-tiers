#!/bin/sh
# E2E test for logistic-bot-tiers with Krastorio 2 but WITHOUT Space Age.
# This is the configuration the "advanced" tier exists for — K2 installed,
# Space Age disabled. Verifies that advanced tier entities/recipes/techs are
# present and turbo is correctly absent.
#
# Usage: tests/run-e2e-k2-nosa.sh
# Env:   FACTORIO_BIN=/path/to/factorio
#        E2E_WORK_DIR=/tmp/logistic-bot-tiers-e2e-k2-nosa
set -e
...
WORK="${E2E_WORK_DIR:-/tmp/logistic-bot-tiers-e2e-k2-nosa}"
...
# Symlink Krastorio2 and its dependencies (same as run-e2e-k2.sh)
for f in \
...
# Explicitly disable Space Age so the advanced tier is active.
# Without this, Factorio auto-enables all installed DLC.
cat > "$WORK/mods/mod-list.json" <<'EOF'
{
  "mods": [
    ...
    { "name": "ChangeInserterDropLane", "enabled": true },
    { "name": "logistic-bot-tiers", "enabled": true },
    { "name": "bt-e2e", "enabled": true }
  ]
}
EOF
...
{
  "name": "bt-e2e-k2-nosa", "description": "", "tags": [], "max_players": 0,
