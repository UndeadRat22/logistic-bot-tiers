#!/bin/sh
# E2E test for logistic-bot-tiers with Krastorio2-spaced-out (K2SO).
# K2SO forces Space Age, so the "superior" tier is the interesting path here —
# turbo is present (Space Age), advanced is absent (requires K2 without SA),
# and superior chains from turbo instead of advanced.
#
# Usage: tests/run-e2e-k2so.sh
# Env:   FACTORIO_BIN=/path/to/factorio
#        E2E_WORK_DIR=/tmp/logistic-bot-tiers-e2e-k2so
set -e
...
WORK="${E2E_WORK_DIR:-/tmp/logistic-bot-tiers-e2e-k2so}"
...
# Symlink Krastorio2-spaced-out, Krastorio2, and all shared dependencies
for f in \
...
{
  "name": "bt-e2e-k2so", "description": "", "tags": [], "max_players": 0,
